...
const SchedulePage: React.FC = () => {
  ...
  const coursesWithoutSchedules = semesterCourses.filter(c => 
    !data.schedules.some(s => s.courseId === c.id)
  );

  const canAddSchedule = coursesWithoutSchedules.length > 0;
  ...
  const handleOpenModal = (sched?: Schedule) => {
    if (sched) {
      ...
      if (coursesWithoutSchedules.length > 0) {
        ...
        setForm({
          courseId: coursesWithoutSchedules[0].id,
          day: 'Senin',
          startTime: '08:00',
          endTime: '10:00',
          room: '',
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
          ...
          title={!canAddSchedule ? (semesterCourses.length === 0 ? "Tambahkan mata kuliah terlebih dahulu" : "Semua mata kuliah sudah memiliki jadwal") : "Tambah Jadwal"}
        >
                ...
                <div className="space-y-1">
                  ...
                  >
                    {editingSchedule ? (
                      ...
                      coursesWithoutSchedules.map(c => <option key={c.id} value={c.id}>{c.name} ({c.code})</option>)
                    )}
                  </select>
                </div>
